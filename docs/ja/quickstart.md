...
各エージェントで、タスクを進める方法を判断するために選択できる送信側ハンドオフのオプション一覧を定義できます。
...
ワークフローが実行され、トリアージ エージェントが 2 つの専門 エージェント間を正しくルーティングすることを確認しましょう。
...
エージェントの実行中に何が起きたかを確認するには、OpenAI ダッシュボードの [Trace ビューアー](https://platform.openai.com/traces) に移動して、エージェント実行のトレースを表示してください。
