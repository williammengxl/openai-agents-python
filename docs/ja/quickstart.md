...
各エージェントで、タスクを前進させるために選択できる送信側ハンドオフの選択肢の一覧を定義できます。
...
ワークフローが実行され、トリアージ エージェントが 2 つの専門エージェントの間で正しくルーティングすることを確認しましょう。
...
エージェント実行中に何が起きたかを確認するには、OpenAI ダッシュボードの Trace viewer に移動し、エージェント実行のトレースを表示してください。
