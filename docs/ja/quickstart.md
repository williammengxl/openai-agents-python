...
各エージェントで、タスクを進める方法を決定するために選択可能な送信側ハンドオフ オプションのインベントリを定義できます。
...
エージェントの実行中に何が起きたかを確認するには、[OpenAI Dashboard の Trace viewer](https://platform.openai.com/traces) に移動してトレースを表示します。
