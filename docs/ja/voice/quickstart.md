...
Agents SDK の基本の [クイックスタート手順](../quickstart.md) に従って仮想環境を設定してください。次に、SDK の音声用オプション依存関係をインストールします。
...
まず、いくつかのエージェントを設定しましょう。これは、この SDK でエージェントを作成したことがあれば馴染みがあるはずです。ここでは、複数のエージェント、ハンドオフ、そしてツールを用意します。
