...
LiteLLM でサポートされるモデルの全一覧は、[litellm プロバイダーのドキュメント](https://docs.litellm.ai/docs/providers)をご覧ください。
...
LiteLLM のレスポンスで Agents SDK の使用状況メトリクスを反映したい場合は、エージェント作成時に `ModelSettings(include_usage=True)` を渡してください。
