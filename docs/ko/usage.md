...
사용량은 실행 중 수행된 모든 모델 호출(도구 호출과 핸드오프 포함)을 기준으로 집계됩니다.
...
`Session`(예: `SQLiteSession`)을 사용할 때, `Runner.run(...)`의 각 호출은 해당 실행에 대한 사용량을 반환합니다. 세션은 컨텍스트 유지를 위해 대화 기록을 보관하지만, 각 실행의 사용량은 독립적입니다.
